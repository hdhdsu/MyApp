...
public class ArrayUtil {
     ...
     */
    public static void sortingByFastRecursion(int[] intArray, int start, int end,
            boolean ascending) {
    ...
    public static void sortingByFastStack(int[] intArray, boolean ascending) {
        ...
        while (!sa.isEmpty()) {
            ...
            if (ascending) {
                while (i < j) {
                    while (intArray[j] > tmp && i < j) {//找到第一个比tmp小的数
                        j--;
                    }
                    if (i < j) {
                        intArray[i] = intArray[j];//设置
                        i++;
                    }
                    for (; intArray[i] < tmp && i < j; i++) {//找到第一个tmp大的id
                        ;
                    }
                    if (i < j) {
                        intArray[j] = intArray[i];//设置
                        j--;
                    }
                }
            } else {
     ...
     * 中分发排序
     */
    public static void sort(ArrayList<Integer> items) {
     ...
     * 牺牲内存式排序
     */
    public static void sort2(ArrayList<Integer> items) {
        ArrayList<Integer>[] list = new ArrayList[getMax(items) + 1];
        for (int i = 0; i < items.size(); i++) {
            int a = items.get(i);
            ArrayList<Integer> item = list[a];
            if (item == null) {
                item = new ArrayList<>();
                list[a] = item;
            }
    ...
    //得出最大的数
    private static int getMax(ArrayList<Integer> items) {
        int max = 0;
        for (int i = 0; i < items.size(); i++) {
            int cur = items.get(i);
            if (cur > max) {
     ...
     */
    public static String toString(int[] integers, String startSymbols, String separator,
            String endSymbols) {
     ...
     * @return 例如开始符号为"{"，分隔符为", "，结束符号为"}"，那么结果为：{1, 2, 3}
     */
    public static String toString(Object[] objects, String startSymbols, String separator,
            String endSymbols) {
        boolean addSeparator = false;
        StringBuffer sb = new StringBuffer();
        //如果开始符号不为null且不空
     ...
     * @return 例如开始符号为"{"，分隔符为", "，结束符号为"}"，那么结果为：{1, 2, 3}
     */
    public static String toString(List<String> objects, String startSymbols, String separator,
            String endSymbols) {
        boolean addSeparator = false;
        StringBuffer sb = new StringBuffer();
        //如果开始符号不为null且不空
